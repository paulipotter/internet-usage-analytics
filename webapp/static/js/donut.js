...
function DonutCharts() {
    ...
    var legends = charts.select('.legend').selectAll('g').data(catNames)
                            .enter().append('g').attr('transform', function(d, i) {
                                    return 'translate(' + (i * 150 + 50) + ', 10)';
                                });
    legends.append('circle').attr('class', 'legend-icon')
                .attr('r', 6)
...
function DonutCharts2() {
    var charts = d3.select('#donut-charts2');
    var chart_m, chart_r, color = d3.scale.category20();
    var getCatNames = function(dataset) {
            var catNames = new Array();
            for (var i = 0; i < dataset[0].data.length; i++) {
...
function genData2() {
    ...
    //     "device": [{"label":Device.objects.get(key=1),"count":Device.objects.filter(key=1).count()},
    //                {"label":Device.objects.get(key=2),"count":Device.objects.filter(key=2).count()},
    //                {"label":Device.objects.get(key=3),"count":Device.objects.filter(key=3).count()},
    //                {"label":Device.objects.get(key=96),"count":Device.objects.filter(key=96).count()},
    //                {"label":Device.objects.get(key=99),"count":Device.objects.filter(key=99).count()}]
    // }
